...
figs_dir = '16_scouts_cycles/no_filtering/';

wave_metrics_matrix_path = '/media/sil2/Data/Yuval O/Rotem Simulations/20230702_RightHemisphereSmallScouts_306716/figures/flip_signal_new/no_filtering/En7_lr_symmetric/Two Scouts - wave metrics - first maxima.mat';
...
close all
save_path = '/media/sil2/Data/Yuval O/Rotem Simulations/figures_for_paper/16_scouts_cycles/En7_lr_symmetric/no_filtering/';
line_width = 3;
...
yticks = get(ax,'YTick');
yticks_labels = get(ax,'YTickLabel');
set(ax,{'YTick','YTickLabel'},{yticks(2:2:end),yticks_labels(2:2:end)})
ylabel(hCbar,'Correlation',{'FontSize','Position'},{8,[4.606666586200397,0.925000071525574,0]});
set(gcf,'PaperPositionMode','auto');print 'PLDC_1-4' -dpdf -painters;
saveJpegAndFig(f,save_path,'PLDC_1-4',1,'saveEPS',1)
...
set(gcf,'PaperPositionMode','auto');print spatial_lag -dpdf -painters;
...
set(gcf,'PaperPositionMode','auto');print electrode_signal -dpdf -painters;
...
plotting_config.fs = 10000;
plotting_config.start_end_wave = []; 
plotting_config.duplicate_hemisphere = 1;
plotting_config.crop_at = [260, 5000];
...
plot(t,electrode_set_unfiltered.electrode_signals(channels_for_signal(1),:),'LineWidth',line_width)
hold on
plot(t,electrode_set_filtered.electrode_signals(channels_for_signal(1),:),'LineWidth',line_width)
legend('Unfiltered signal','Filtered signal')
xlabel('Time [s]')
ylabel('Electrode Simulated Signal [AU]')
set(gcf,'PaperPositionMode','auto');print filtered_vs_non_filtered_signal -dpdf -painters;
saveJpegAndFig(f1,save_path,'filtered_vs_non_filtered_signal',1,'saveEPS',1)